...
;	startY = 10
	...
	For i = 0 To n-1
		;newX# = offsetStep(0,0, 2*dis,i*phi, 1)
		;newY# = offsetStep(0,0, 2*dis,i*phi, 2)
		dis2# = 2*dis*Sqr(1+dis^2)
		newX# = dis2*Cos(i*theta)
		newY# = dis2*Sin(i*theta)
		
		;createPoint(newX,newY, 0,0,255)
		
		For i2 = 0 To n-1
			;tempX2# = offsetStep(0,0, dis,i2*phi, 1)
			;tempY2# = offsetStep(0,0, dis,i2*phi, 2)
			tempX2# = dis*Cos(i2*theta)
			tempY2# = dis*Sin(i2*theta)
			
			Local newXY#[2]
			translateXY(tempX2,tempY2, newX,newY, newXY)
			
			newX2# = newXY[1]
			newY2# = newXY[2]
			
			polygon_oriented(newX2,newY2, newX,newY, rad2, k)
		Next
	Next
