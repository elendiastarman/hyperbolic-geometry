...
;		p\tv = transform(pxt,pyt, 2)
		...
		testU# = transform(testTXY[1],testTXY[2], 1)
		testV# = transform(testTXY[1],testTXY[2], 2)
		
		Color 200,200,0 ;p\r/1.5,p\g/1.5,p\b/1.5
		circ(R*testU+gw/2,R*testV+gh/2, 3, 1)
...
;	M[1] = x1 : M[2] = -x2 : M[3] = -x3 : R[1] = 0
;	M[4] = x1 : M[5] = x2 : M[6] = x3 : R[2] = 0
;	M[7] = -x3+x1*x1*x3 : M[8] = x1*x2*x3 : M[9] = x2*x3*x3 : R[3] = (-x2*x2-x1^4+x1*x1*x2*x2+x2*x2*x3*x3+x3*x3-1+2*x1*x1)/2.
	
;	linearSystem3(M,R,sol)
...
;	c# = x3 ;x3/(2*x1*x1-1)
	...
	;fpos# = (c*x1*x2 + Abs(x3)*Sqr(x2*x2+x3*x3-c*c))/(x2*x2+x3*x3)
	;fneg# = (c*x1*x2 - Abs(x3)*Sqr(x2*x2+x3*x3-c*c))/(x2*x2+x3*x3)
	;i# = (c*x1-fneg*x2)/x3
	
	;f# = Abs(x3)*Sqr(1./(x2*x2+x3*x3))
	;i# = -f*x2/x3
	
;	fpos# = (x1*x2*x3 + x1*x3*Sqr(x2*x2-x3*x3*(1-x3*x3)))/(x2*x2+x3*x3)
;	fneg# = (x1*x2*x3 - x1*x3*Sqr(x2*x2-x3*x3*(1-x3*x3)))/(x2*x2+x3*x3)
;	i# = -x1 - fneg*x2/x3
...
;	ipos# = 1 + x2*Sqr(x3*x3+1)/x3
;	ineg# = 1 - x2*Sqr(x3*x3+1)/x3
...
;	f# = -1.0
	...
	fpos# = (x1*x2*x3 + (x2*x3))/(x2*x2+x3*x3)
	fneg# = (x1*x2*x3 - (x2*x3))/(x2*x2+x3*x3)
	...
	debug = 0
	If debug
		DebugLog "Test of correctness (should be 1,0,0): "+ct+", "+cx+", "+cy
		DebugLog "0.0? "+(1-ct*ct+cx*cx+cy*cy)
		
		DebugLog ""
		DebugLog "1.0? "+(a*a-d*d-g*g)
		DebugLog "-1.0? "+(b*b-e*e-h*h)
		DebugLog "-1.0? "+(c*c-f*f-i*i)
		DebugLog "0.0? "+(a*b-d*e-g*h)
		DebugLog "0.0? "+(a*c-d*f-g*i)
		DebugLog "0.0? "+(b*c-e*f-h*i)
	EndIf
